...
static int oplus_wired_update_chg_output_state(struct oplus_chg_wired *chip, int state)
{
	struct mms_msg *msg;
	int rc;

	chg_info("set state = %d\n", state);
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM,
				      WIRED_ITEM_CHG_OUTPUT_STATE, state);
	if (!msg) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
	...
	rc = oplus_mms_publish_msg(chip->wired_topic, msg);
	if (rc < 0) {
		chg_err("publish chg suspend msg error, rc=%d\n", rc);
		kfree(msg);
	}
...
static int oplus_wired_update_chg_input_state(struct oplus_chg_wired *chip, int state)
{
	struct mms_msg *msg;
	int rc;
	...
	chg_info("set state = %d\n", state);
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM,
				      WIRED_ITEM_CHG_INPUT_STATE, state);
	if (!msg) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
	...
	rc = oplus_mms_publish_msg(chip->wired_topic, msg);
	if (rc < 0) {
		chg_err("publish chg disable msg error, rc=%d\n", rc);
		kfree(msg);
	}
						   ...
						   bool step)
{
	...
	oplus_wired_update_chg_input_state(chip, disable);
						    ...
						    bool step)
{
	...
	chg_info("charging disabled change to %s by %s\n",
		 disable ? "true" : "false", client);
	oplus_wired_update_chg_output_state(chip, disable);
