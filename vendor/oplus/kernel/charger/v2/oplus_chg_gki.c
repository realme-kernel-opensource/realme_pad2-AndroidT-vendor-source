...
struct oplus_gki_device {
	...
	struct delayed_work gauge_current_update_work;
	...
	bool power_supply_change_pending;
	int current_now;
};
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		pval->intval = chip->current_now;
		schedule_delayed_work(&chip->gauge_current_update_work, 0);
		break;
...
#define OPLUS_POWER_SUPPLY_CHANGED_NOTIFY_INTERVAL_MS	300
static void oplus_gki_gauge_current_update_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_gki_device *chip =
		container_of(dwork, struct oplus_gki_device, gauge_current_update_work);
	union mms_msg_data data = { 0 };
...
static void oplus_gki_power_supply_change_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_gki_device *chip =
		container_of(dwork, struct oplus_gki_device, power_supply_change_work);

	if (chip->power_supply_change_pending && chip->batt_psy) {
		power_supply_changed(chip->batt_psy);
		chip->power_supply_change_pending = false;
		schedule_delayed_work(&chip->power_supply_change_work,
				      OPLUS_POWER_SUPPLY_CHANGED_NOTIFY_INTERVAL_MS);
	}
}

static void oplus_gki_battery_power_supply_changed(struct oplus_gki_device *chip,
						  bool emergent)
{
	...
	if (emergent) {
		...
		schedule_delayed_work(&chip->power_supply_change_work,
				      OPLUS_POWER_SUPPLY_CHANGED_NOTIFY_INTERVAL_MS);
	} else if (!chip->power_supply_change_pending) {
		chip->power_supply_change_pending = true;
		schedule_delayed_work(&chip->power_supply_change_work, 0);
	}
}
...
static void oplus_gki_gauge_update_work(struct work_struct *work)
{
	...
	if (eng_version == AGING || eng_version == HIGH_TEMP_AGING ||
		...
		oplus_gki_battery_power_supply_changed(chip, false);
	} else {
		if (time_is_before_jiffies(update_time)) {
			update_time = jiffies + FORCE_UPDATE_TIME * HZ;
			oplus_gki_battery_power_supply_changed(chip, true);
		}
	}
}
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_ONLINE:
			...
			if (!chip->wired_online) {
				...
				chip->pre_wired_type = OPLUS_CHG_USB_TYPE_UNKNOWN;
				chip->pre_batt_status = POWER_SUPPLY_STATUS_UNKNOWN;
				chip->power_supply_change_pending = false;
			} else {
				...
				if (chip->wired_online != chip->pre_wired_online)
					chip->power_supply_change_pending = true;
			}
			chg_info("psy_type=%d, wired_online=%d, pre_wired_online = %d\n",
				  usb_psy_desc.type, chip->wired_online,
				  chip->pre_wired_online);

			chip->pre_wired_online = chip->wired_online;
			if (!chip->wired_online)
				oplus_gki_battery_power_supply_changed(chip, true);
			break;
					 ...
					 enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case COMM_ITEM_BATT_STATUS:
			...
			if (chip->batt_psy && chip->pre_batt_status != chip->batt_status) {
				oplus_gki_battery_power_supply_changed(chip,
...
static __init int oplus_chg_gki_init(void)
{
	...
	INIT_DELAYED_WORK(&gki_dev->gauge_current_update_work, oplus_gki_gauge_current_update_work);
	INIT_DELAYED_WORK(&gki_dev->power_supply_change_work, oplus_gki_power_supply_change_work);
