...
struct oplus_chg_comm {
	...
	struct work_struct chg_state_change_work;
					   ...
					   enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_CHG_TYPE:
			...
			break;
		case WIRED_ITEM_CHG_INPUT_STATE:
			schedule_work(&chip->chg_state_change_work);
			break;
		case WIRED_ITEM_CHG_OUTPUT_STATE:
			schedule_work(&chip->chg_state_change_work);
			break;
...
static void oplus_comm_chg_type_change_work(struct work_struct *work)
{
	...
	chg_debug("start the gauge_check_work\n");
...
static void oplus_chg_state_change_work(struct work_struct *work)
{
	struct oplus_chg_comm *chip =
			container_of(work, struct oplus_chg_comm, chg_state_change_work);
...
static int oplus_comm_driver_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&comm_dev->chg_state_change_work, oplus_chg_state_change_work);
