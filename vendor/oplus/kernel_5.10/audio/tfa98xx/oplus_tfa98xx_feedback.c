...
#define SMARTPA_ERR_FB_VERSION             "1.0.1"
#define SPK_ERR_FB_VERSION                 "1.0.1"
...
static int tfa98xx_check_result(struct tfa98xx *tfa98xx, uint8_t *pdata)
{
	...
	for (index = 0; index < tfa_fb.pa_cnt; index++) {
		...
		if (tfa_fb.low_pr && tfa_fb.lib_new) {
			at = TFA_GET_AT(pdata, index);
			if (!low_pressure && (at < tfa_fb.at_min)) {
				SetFdBuf(lp_buf, "pressure@@%u", at);
				low_pressure = true;
			}
			pr_info("speaker%d, atmospheric pressure = %u", index+1, at);
		}
		...
		if ((false == spk_err) && (tfa_fb.damage_flag & (1 << index)) && (fr < F0_BLOCK_HOLE)) {
			if (0 == tfa_fb.last_damage_r0[index]) {
				tfa_fb.last_damage_r0[index] = re;
			} else if (re == tfa_fb.last_damage_r0[index]) {
				SetFdBuf(fb_buf, "TFA98xx SPK%u:speaker short circuit or open circuit.R0=%u,R0_cal=%u;", \
						index+1, re, tfa_fb.r0_cal[index]);
				spk_err = true;
				tfa_fb.last_damage_r0[index] = 0;
